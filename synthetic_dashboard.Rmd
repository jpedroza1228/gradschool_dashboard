...
uo %>% 
  ...
  mutate(mean_fte = mean(ge_teach_fte, na.rm = TRUE), #Law was NA without this argument
...
uo %>%
  filter(ge == 1) %>% 
  pivot_longer(cols = c(8, 10, 12), # Consider using column names to prevent unintended outcomes c(ge_teach_fte, ge_rsch_fte, ge_admin_fte)
               names_to = 'fte_ge',
...
# filter(amount_fte > 0) %>% not sure if you need to filter out 0.0 FTE
...
#  geom_smooth(aes(color = as.factor(course_dept)), method = 'lm', se = FALSE, size = 1) + #I would consider removing these throughout
  theme(legend.position = 'none') +
