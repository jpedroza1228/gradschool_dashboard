...
uo <- read_csv('C:/Users/cpppe/Desktop/github_projects/gradschool_dashboard/do_not_push/data/wide_data.csv') %>% 
  filter(year == 18) %>% 
  dplyr::select(-X1)
...
uo %>%
  ...
  geom_density_ridges(alpha = .3, color = 'gray70', scale = .9) +
...
```{r}
# make sure colors match for both plots
...
ggarrange(cod1, cod2, common.legend = TRUE)
...
ggarrange(coe1, coe2, common.legend = TRUE)
...
uo %>% 
  filter(ge_teach == 1) %>% 
  drop_na(ge_teach, ge_teach_fte, credit, fte) %>% 
